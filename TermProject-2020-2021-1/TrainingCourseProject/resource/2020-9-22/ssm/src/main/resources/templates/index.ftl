...
<table>
    ...
    <#if list?? && (list?size > 0) >
    <tr><td><input type="checkbox"  name="ids" onclick="choose();">全选/非全选</td><td>序号</td><td>user_id</td><td>user_name</td><td>user_score</td></tr>
    <#list list as item>
...
-->
       ...
       <!-- <tr style="background-color:<#if item.userScore gt 90>red<#elseif item.userScore gt 80>yellow<#else>blue</#if>"><td>${item_index+1}</td><td>${item.userId}</td><td>${item.userName}</td><td>${item.userScore}</td></tr>-->
        ...
        <tr><td><input type="checkbox" name="ids" onclick="choose1();" value="${item.userId}"></td><td>${item_index+1}</td><td>${item.userId}</td><td>${item.userName}<td>${item.userScore}</td></tr>
    </#list>
