        ...
        using (SqlConnection conn = new SqlConnection(connString))
        {
            using (SqlDataAdapter apter = new SqlDataAdapter("select * from Table_1", conn))
            {
                ...
                for (int i = 0; i < da.Rows.Count; i++)
                {
                    ...
                    sb.AppendFormat("<tr><td>{0}</td><td>{1}</td><td>{2}</td><td>{3}</td><td>{4}</td><td><a href='userInfo.ashx?id={5}'> 详 细 </a></td><td><a href='delete.ashx?id={5}' >删除</a></td><td ><a href='Edit.ashx?id={5}'>编辑</a></td></tr>", da.Rows[i]["SN"], da.Rows[i]["UserName"], da.Rows[i]["UserPass"], Convert.ToDateTime(da.Rows[i]["RegTime"]).ToShortDateString(), da.Rows[i]["Email"], da.Rows[i]["SN"]);
