...
function ShowDivContent(PageUrl, objXmlHttp, CurrentDivID, objShowPanel) {//alert(PageUrl);
    ...
    if (window.XMLHttpRequest) {
        objXmlHttp.onload = function () {
            ...
            objXmlHttp = null;
            switch (CurrentDivID) {
                case "DivPaiHangFlag1":
                case "DivPaiHangFlag2":
                case "DivPaiHangFlag3":
                    SwapPaiHangCircleDiv(CurrentDivID, '1');
                    break;
                case "DivCircleAction1":
                case "DivCircleAction2":
                    SwapCircleActionDiv(CurrentDivID, '1');
                    break;
                case "DivUserPanel":
                    //_gObjByID('DivUserPanel_' + CurrentUserPanelDiv +"_Content").innerHTML=objShowPanel.innerHTML;
                    break;
                case "DivShopPaiHangFlag1":
                case "DivShopPaiHangFlag2":
                case "DivShopPaiHangFlag3":
        ...
        objXmlHttp.onreadystatechange = function () {
            if (objXmlHttp.readyState == 4) {
                if (objXmlHttp.status == 200) {//alert(objXmlHttp.responseText);
                    objShowPanel.innerHTML = objXmlHttp.responseText;
                    objXmlHttp = null;
                    switch (CurrentDivID) {
                        case "DivPaiHangFlag1":
...
//展示页面图片
function ShowImg(ImgId, ImgSrc) {//alert(_gObjByID(ImgId).src);
...
function OperateSearchDiv(divID) {	//alert(divID);
...
function OperatePaiHangDiv(divID) {
    ...
    if (CurrentShopPaiHangFlag != divID) {
        ...
        if (CurrentShopPaiHangFlag != "") {
            ...
            objShowPanel.innerHTML = GetLoadingImg();
            if (obj.innerHTML == null || obj.innerHTML == "" || obj.innerHTML == GetLoadingImg()) {//alert('GetFromAspx');
//		            var oXmlHttp = createXMLHttp();		
...
function SwapPaiHangCircleDiv(DivKey, ID) {//alert(DivKey);
    for (var i = 1; i <= 9; i++) {
...
function SwapCircleActionDiv(DivKey, ID) {//alert(DivKey);
    for (var i = 1; i <= 5; i++) {
        _gObjByID(DivKey + '_S' + i).style.display = 'block';
        _gObjByID(DivKey + '_B' + i).style.display = 'none';
...
function SwapPaiHangShopDiv(DivKey, ID) {//DivS1  DivB1
    for (var i = 1; i <= 9; i++) {
...
function SwapGoAndWantShopDiv(DivKey, ID) {//DivS1  DivB1
    for (var i = 1; i <= 10; i++) {
        _gObjByID(DivKey + '_S' + i).style.display = 'block';
        _gObjByID(DivKey + '_B' + i).style.display = 'none';
    }
