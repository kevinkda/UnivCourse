...
<head>
    ...
    <script>
        ...
        function del() {
            //询问框，问用户是否需要删除的操作!
            var userAddressId = document.getElementById("userAddressId").val();
            if (confirm('你确定需要删除第' + userAddressId + '个的数据吗?')) {
                // location.href = 'del?userAddressId=' + userAddressId+'userEmail=' + userEmail;
                alert("id" + userAddressId);
            } else {
                alert('取消删除');
            }
        }
        ...
        $(function () {
            $.ajax({
                ...
                success: function (data) {
                    ...
                    // }
                    for (var i = 0; i < data.length; i++) {
                        if (data[i].level == 'province') {
                            areaName[i] = data[i].area_name;
                            areaId[i] = data[i].area_id;
                        }
                    }
                    // <option value="null">请选择省</option>
                    var str = "<option value=\"null\" selected>请选择省</option>";
        ...
        function userLogin() {
            var userPro = $(".selectProvince").find("option:selected").text();
            var userCity = $(".selectCity").find("option:selected").text();
            var userArea = $(".selectArea").find("option:selected").text();
            ...
            $(function () {
                $.ajax({
                    ...
                    url: 'addressInsert',
                    data: {
                        'userAddressPeople': $("#user-name").val().trim(),
                        'userPhone': $("#user-phone").val().trim(),
                        'userEmail': $("#userEmail").val().trim(),
                        'userAddress': userPro + userCity + userArea + address,
                        'userAddressStatus': 0
                    },
                    async: false,
                    success: function (data) {
                    ...
                    <#if list?? && (list?size > 0)>
                        <#list list as list1>
                            <#if  1 == '${list1.userAddressStatus}'>
                                <input type="hidden" id="userAddressId" value="${list1.userAddressId}">
                            ...
                            <#else>
                                <li class="user-addresslist">
                                    <input type="hidden" id="userAddressId" value="${list1.userAddressId}">
                                    <span class="new-option-r"><i class="am-icon-check-circle"></i><a
                                                href="updateUserAddress?userAddressId=${list1.userAddressId}&userEmail=${list1.userEmail}">设为默认</a></span>
                                    <p class="new-tit new-p-re">
                                        <span class="new-txt">${list1.userAddressPeople}</span>
                                        <span class="new-txt-rd2">${list1.userPhone}</span>
                                    </p>
                ...
                <div class="am-modal am-modal-no-btn" id="doc-modal-1">
                    ...
                    <div class="add-dress">
                        ...
                        <div class="am-u-md-12 am-u-lg-8" style="margin-top: 20px;">
                            <form class="am-form am-form-horizontal" id="form1" action="addressInsert" method="post">

                                <div class="am-form-group">
                                    <label for="user-name" class="am-form-label">收货人</label>
                                    <div class="am-form-content">
                                        <input type="text" id="user-name" name="userAddressPeople" placeholder="收货人">
                                    </div>
                                </div>
                                ...
                                <div class="am-form-group">
                                    ...
                                    <div class="am-form-content">
                                        <input id="user-phone" name="userPhone" placeholder="手机号必填" type="email">
                                    </div>
                                </div>
                                <div class="am-form-group">
                                    <label for="user-address" class="am-form-label">所在地</label>
                                    <div class="am-form-content address">
                                        <select data-am-selected class="selectProvince" onchange="changeProvince()">
                                            <option value="null">请选择省</option>
                                        </select>
                                        <select data-am-selected class="selectCity" onchange="changeCity()">
                                            <option value="null" selected>请选择市（州）</option>
                                        </select>
                                        <select data-am-selected class="selectArea">
                                            <option value="null">请选择县（区）</option>
                                        </select>
                                    </div>
                                </div>
                                <#if Session["user"]?exists>
                                ...
                                <div class="am-form-group">
                                    <div class="am-u-sm-9 am-u-sm-push-3">
                                        <a class="am-btn am-btn-danger"><input class="am-btn am-btn-danger"
