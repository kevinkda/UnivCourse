 ...
 * <p>数据分页部分依赖{@link PageBean}，自1.5版本开始本类方法提供的数据分页将被标记为{@link Deprecated}</p>
 * @since 1.0 (JDK 1.7)
 */
...
//    批处理提交方法
    ...
    public int addBatch(List<Object[]> args) {
        int tempCount = args.size() + intBatchCount;
        try {
            if (tempCount > BATCH_MAX) {
//                int[] group = calc(args.size());
//                for (int i = 0; i < group.length; i++) {
//                    for (int j = 0; j < group[i]; j++) {
//                        ;
//                    }
//                }
                int temp1 = tempCount % BATCH_MAX;
            } else {
                for (Object[] objects :
                        args) {
                    addBatch(objects);
    ...
    private int[] calc(int count) {
        ...
        int temp = BATCH_MAX - intBatchCount;
        ints.add(temp);
        while (temp < count) {
            temp += BATCH_MAX;
        }
        ints.add(count);
        int[] returnCount = new int[ints.size()];
        for (int i = 0; i < ints.size(); i++) {
            returnCount[i] = ints.get(i);
        }
        return returnCount;
    }
